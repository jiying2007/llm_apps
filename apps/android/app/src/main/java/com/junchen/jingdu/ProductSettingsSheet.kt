...
@Composable internal fun ProductSettingsSheet(state: AppUiState, actions: JingduActions) {
    ...
    ModalBottomSheet(onDismissRequest = actions.onClosePanel) {
        LazyColumn(modifier = Modifier.fillMaxWidth().fillMaxHeight(0.9f), contentPadding = PaddingValues(start = 20.dp, end = 20.dp, bottom = 36.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
            ...
            item { SettingSection(stringResource(R.string.page_tone)) { Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { ReaderPalette.entries.forEach { p -> FilterChip(settings.palette == p, { actions.onSettingsChanged(settings.copy(palette = p)) }, label = { Text(stringResource(when (p) { ReaderPalette.PAPER -> R.string.paper; ReaderPalette.LIGHT -> R.string.light; ReaderPalette.NIGHT -> R.string.night })) }) } } } }
            item { SettingSection(stringResource(R.string.font)) { Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { ReaderTypeface.entries.forEach { f -> FilterChip(settings.typeface == f, { actions.onSettingsChanged(settings.copy(typeface = f)) }, label = { Text(stringResource(if (f == ReaderTypeface.SYSTEM) R.string.system_font else R.string.serif)) }) } } } }
            ...
            item { SettingSection(stringResource(R.string.auto_page)) { ReaderSlider(stringResource(R.string.interval), settings.autoPageDelayMs.toFloat(), 2500f..15000f, stringResource(R.string.seconds_value, settings.autoPageDelayMs / 1000f)) { actions.onSettingsChanged(settings.copy(autoPageDelayMs = it.toLong())) }; OutlinedButton(onClick = actions.onToggleAutoPaging, modifier = Modifier.fillMaxWidth()) { Text(stringResource(if (state.autoPaging) R.string.stop_auto_page else R.string.start_auto_page)) } } }
            item { SettingSection(stringResource(R.string.sleep_timer)) { Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { listOf(0, 15, 30, 60).forEach { m -> FilterChip(state.sleepMinutes == m, { actions.onSleepTimer(m) }, label = { Text(if (m == 0) stringResource(R.string.off) else stringResource(R.string.minutes_value, m)) }) } } } }
            item { HorizontalDivider() }
            item { ElevatedCard(Modifier.fillMaxWidth()) { Column(Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) { Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { Icon(Icons.Outlined.CloudOff, null, tint = MaterialTheme.colorScheme.primary); Text(stringResource(R.string.local_asset_backup), fontWeight = FontWeight.SemiBold); AssistChip(onClick = {}, label = { Text("Pro") }) }; Text(stringResource(R.string.local_asset_backup_body), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant); if (state.proUnlocked) Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { OutlinedButton(onClick = actions.onExportBackup, modifier = Modifier.weight(1f)) { Icon(Icons.Outlined.FileUpload, null); Spacer(Modifier.width(6.dp)); Text(stringResource(R.string.export_backup)) }; OutlinedButton(onClick = actions.onImportBackup, modifier = Modifier.weight(1f)) { Icon(Icons.Outlined.FileDownload, null); Spacer(Modifier.width(6.dp)); Text(stringResource(R.string.restore_backup)) } } else OutlinedButton(onClick = actions.onUpgradePro, modifier = Modifier.fillMaxWidth()) { Icon(Icons.Outlined.WorkspacePremium, null); Spacer(Modifier.width(8.dp)); Text(stringResource(R.string.unlock_pro_backup)) } } } }
