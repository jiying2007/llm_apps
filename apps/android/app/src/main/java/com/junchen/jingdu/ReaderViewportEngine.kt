...
internal data class ReaderPresentedText(val sourceText: String, val displayText: String, val map: SourceDisplayMap)
...
internal object ReaderPresentationPipeline {
    fun present(source: String, settings: ReaderSettings): ReaderPresentedText {
        ...
        if (settings.compressBlankLines) presented = presented.replace(Regex("\\n[ \\t]*\\n(?:[ \\t]*\\n)+"), "\n\n")
        if (settings.paragraphSpacingEm > 0f) presented = presented.replace("\n\n", "\n${ReaderTypographySpec.PARAGRAPH_SPACER}\n")
        ...
        return ReaderPresentedText(source, display, SourceDisplayMap.compose(sourceToPresented, presentedToDisplay))
    }

    fun key(settings: ReaderSettings): Int = listOf(
...
/** Bounded presentation pipeline shared by paged, continuous, skim and selection paths. */
internal class ReaderViewportEngine(context: Context, private val bookId: String) : Closeable {
    ...
    fun readAround(position: Long, settings: ReaderSettings): ReaderDisplayWindow {
        ...
        val key = WindowKey(aligned, ReaderPresentationPipeline.key(settings))
        ...
        val result = ReaderDisplayWindow(aligned, source, presented.displayText, length, presented.map)
        cache[key] = result
        return result
    }
    ...
    @Synchronized override fun close() { cache.clear(); reader.close() }

    private data class WindowKey(val start: Long, val presentationKey: Int)
    private companion object { const val MAX_WINDOWS = 8; const val ALIGN_CHARS = 2048L; const val BACK_BUFFER_CHARS = 1536L }
}

data class PageLayoutKey(val textHash: Int, val widthPx: Int, val heightPx: Int, val typographyFingerprint: Int, val columns: Int)
data class PageLayoutSnapshot(val displayedEndUtf16: Int, val displayedCodePoints: Long, val sourceCodePoints: Long, val firstColumnEndUtf16: Int)
...
internal object ReaderPageLayoutCache {
    ...
    fun measure(sourceText: String, displayText: String, widthPx: Int, heightPx: Int, columns: Int, settings: ReaderSettings, density: Density, typeface: Typeface? = null, map: SourceDisplayMap? = null): PageLayoutSnapshot {
        ...
        val key = PageLayoutKey(31 * sourceText.hashCode() + displayText.hashCode(), widthPx, heightPx, spec.fingerprint, safeColumns)
        get(key)?.let { return it }
        val paint = TextPaint(TextPaint.ANTI_ALIAS_FLAG or TextPaint.SUBPIXEL_TEXT_FLAG).apply {
            textSize = with(density) { spec.fontSizeSp.sp.toPx() }; letterSpacing = spec.letterSpacingEm
            this.typeface = typeface ?: Typeface.create(Typeface.SANS_SERIF, if (spec.weight == ReaderFontWeight.NORMAL) Typeface.NORMAL else Typeface.BOLD)
        }
        ...
        fun endFor(text: CharSequence): Int {
            ...
            val builder = StaticLayout.Builder.obtain(text, 0, text.length, paint, columnWidth)
                .setIncludePad(false).setEllipsize(TextUtils.TruncateAt.END).setMaxLines(Int.MAX_VALUE)
                .setLineSpacing(0f, spec.lineHeightMultiplier.coerceAtLeast(1f)).setAlignment(Layout.Alignment.ALIGN_NORMAL)
        ...
        val secondEnd = if (safeColumns == 2 && firstEnd < layoutText.length) firstEnd + endFor(layoutText.subSequence(firstEnd, layoutText.length)) else firstEnd
