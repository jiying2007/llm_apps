...
void check(bool condition, const char* message) { if (!condition) fail(message); }
std::string take(jd_buffer* buffer) {
  std::string value(buffer->data ? buffer->data : "", static_cast<size_t>(buffer->size));
  jd_buffer_free(buffer); return value;
}
std::string detect(const uint8_t* data, size_t size, uint32_t flags = 0) {
  char name[32]{};
  check(jd_detect_encoding(data, size, flags, name, sizeof(name)) == JD_OK, "encoding detection status");
  return name;
}
}  // namespace
...
int main() {
  ...
  check(std::string(hash) == "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad", "SHA known vector");
  ...
  check(detect(truncatedUtf8, sizeof(truncatedUtf8), JD_DETECT_SAMPLE_TRUNCATED) == "UTF-8", "truncated UTF-8 sample boundary");
  ...
  { std::ofstream file(path, std::ios::binary); file << "第一章 开始\nhello 世界\nChapter 2 Next\nhello again\n"; }
  check(jd_file_sha256(path, hash, sizeof(hash)) == JD_OK, "file SHA status");
  check(std::string(hash).size() == 64, "file SHA length");
  char revision1[JD_SHA256_HEX_SIZE]{}, revision2[JD_SHA256_HEX_SIZE]{};
  check(jd_repair_revision(hash, "hello\x1fhi", revision1, sizeof(revision1)) == JD_OK, "revision status");
  check(jd_repair_revision(hash, "hello\x1fhi", revision2, sizeof(revision2)) == JD_OK, "revision repeat status");
  ...
  check(jd_search(handle, "hello", 10, &buffer) == JD_OK && take(&buffer).find("hello") != std::string::npos, "search");
  check(jd_chapters(handle, 100, &buffer) == JD_OK, "chapters status");
  const std::string chapters = take(&buffer);
  check(chapters.find("第一章") != std::string::npos && chapters.find("Chapter 2") != std::string::npos, "chapters content");
  check(jd_speech_chunk(handle, 0, 20, &buffer) == JD_OK && take(&buffer).find('\t') != std::string::npos, "speech chunk");

  const char* repairedPath = "jingdu-core-repaired.txt";
  check(jd_export_rules(handle, "hello\x1fhi", repairedPath) == JD_OK, "repair export");
  std::ifstream repaired(repairedPath, std::ios::binary);
  const std::string content((std::istreambuf_iterator<char>(repaired)), std::istreambuf_iterator<char>());
  check(content.find("hi 世界") != std::string::npos && content.find("hello") == std::string::npos, "repair content");

  jd_close(handle);
  std::remove(path); std::remove(repairedPath);
  return 0;
}
